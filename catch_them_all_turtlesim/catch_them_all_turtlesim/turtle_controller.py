...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("catch_nearest_or_not",True)
        self.declare_parameter("speed_factor",1.5)
        self.speed_factor=self.get_parameter("speed_factor").value
        self.catch_nearest = self.get_parameter("catch_nearest_or_not").value
    ...
    def callback_alive_turtles(self, msg: TurtleArray):
        if len(msg.turtles) > 0:
            if self.catch_nearest:
                closest_turtle=None
                least_dist = None
                for turtle in msg.turtles:
                    distx= turtle.x-self.pose_.x
                    disty= turtle.y-self.pose_.y
                    dist= math.sqrt((distx*distx)+(disty*disty))
                    if closest_turtle==None or dist < least_dist:
    ...
    def control_loop(self):
        ...
        if distance > 0.2:
            # position
            cmd.linear.x = self.speed_factor*distance
            ...
            if diff > math.pi:
                ...
                diff += 2*math.pi
            cmd.angular.z = self.speed_factor*3*diff
        else:
            # target reached
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
